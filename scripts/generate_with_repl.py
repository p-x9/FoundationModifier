...
def get_swift_header(class_name: str) -> str:
    script = f"/bin/bash -c 'echo -e \"import Foundation\n:type lookup {class_name}\" | swift repl"
...
def generate(class_name: str) -> None:
    ...
    for i, line in enumerate(lines):
        new_class_name = class_name.replace('NS', '')
        if f'class {class_name}' in line or f'protocol {class_name}' in line or f'class {new_class_name}' in line or f'protocol NS{new_class_name}' in line:
            is_struct = False
    ...
    for line in lines:
        if line.startswith('@') and ('var ' not in line) and ('func ' not in line):
            ...
            if ('@available' in attributes and 'renamed' in attributes) or ('@available' in line and 'renamed:' in line) or 'override ' in line:
                current_attributes = []
                continue
            if ('@available' in attributes and 'unavailable' in attributes) or ('@available' in line and 'unavailable' in line) or 'override ' in line:
                current_attributes = []
                continue
    ...
    output_lines = list(filter(lambda line: 'private ' not in line, output_lines))
